...
def build_text_dataloader(
    ...
    remote_path = cfg.dataset.get('remote', None)
    if remote_path and remote_path.startswith("azure://"):
        try:
            from azure.identity import ChainedTokenCredential, AzureCliCredential, DefaultAzureCredential
            from azure.storage.blob import BlobServiceClient
        except ImportError as e:
            raise ImportError(
                "Azure SDK for Python is not installed. Please install it with `pip install azure-identity azure-storage-blob`."
            ) from e
        try:
            full_url = remote_path[len("azure://"):]
            account_url_parts = full_url.split("/", 1)
            account_url = f"https://{account_url_parts[0]}"
            container_blob_path = account_url_parts[1]
            container_name = container_blob_path.split("/")[0]
            ...
            # Validate access (try listing blobs under path)
            _ = list(container_client.list_blobs(name_starts_with=container_blob_path.split("/", 1)[1]))
            print(f"[INFO] Azure blob access verified: {remote_path}")
        except Exception as e:
            raise RuntimeError(f"Failed to access Azure blob at {remote_path}: {e}")
    # build streams
