...
def get_latest_ckpt_path(save_folder: str) -> Optional[str]:
    ckpt_path = os.path.join(save_folder, 'latest-rank0.pt')
...
def main(cfg: DictConfig,
    ...
    # Dataloaders
    print(f"Remote path: {cfg.get('data_remote', None)}")
    ...
    if cfg.get('save_folder'):
        print(cfg.save_folder)
        print(get_latest_ckpt_path(cfg.save_folder))
        latest_ckpt_path = get_latest_ckpt_path(cfg.save_folder)
        if latest_ckpt_path:
            print(f"[INFO] Found latest checkpoint at {latest_ckpt_path}, resuming...")
            cfg.load_path = latest_ckpt_path
        else:
